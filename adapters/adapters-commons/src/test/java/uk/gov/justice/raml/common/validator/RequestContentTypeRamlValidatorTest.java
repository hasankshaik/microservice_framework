...
public class RequestContentTypeRamlValidatorTest {
    ...
    public void shouldIgnoreInvalidMediaTypesInNonPOSTActions() throws Exception {
        ...
        validator.validate(
                raml()
                        .with(resource()
                                .with(httpAction(GET, "application/vnd.structure.dummy.command1+json"))
                                .with(httpAction(POST, "application/vnd.structure.command.command2+json"))
                                .with(httpAction(HEAD, "application/vnd.structure.dummy.command3+json"))
                                .with(httpAction(PUT, "application/vnd.structure.dummy.command4+json"))
                                .with(httpAction(OPTIONS, "application/vnd.structure.dummy.command5+json"))
                        )
                        .build());
    ...
    public void shouldThrowExceptionIfNotvalidMediaType2() throws Exception {
        ...
        validator.validate(
                raml()
                        .with(resource()
                                .with(httpAction(POST, "nd.people.unknown.command1+nonjson")))
                        .build());
