...
import static uk.gov.justice.services.adapters.rest.generator.Names.resourceImplementationNameOf;
 ...
 */
class ActionMapperGenerator extends AbstractInternalGenerator {
    ...
    private CodeBlock mapperConstructorCodeFor(final Resource resource) {
        ...
        resource.getActions().values().forEach(ramlAction -> {
            ...
            actionMappings.forEach(m -> {
                ...
                constructorCode.addStatement("add($S, $S, $S)", buildResourceMethodName(ramlAction,
                        ramlAction.getType() == POST ? new MimeType(mediaType) : null), mediaType, m.getName());
            });
    ...
    private String mapperClassNameOf(final Resource resource) {
        return new StringBuilder().append(resourceImplementationNameOf(resource)).append("ActionMapper").toString();
    }
}